...
SRC_URI="http://www.openwall.com/john/g/${MY_P}.tar.gz
	...
	mpi? ( ftp://ftp.openwall.com/john/contrib/mpi/2010-magnum/${MY_P}-${MPI}.diff.gz )"
...
src_prepare() {
	if use mpi; then
		epatch "${WORKDIR}/${MY_P}-${MPI}.diff"
	fi
	if ! use minimal; then
		epatch "${WORKDIR}/${MY_P}-${JUMBO}.diff"
	fi
	local PATCHLIST="${PATCHLIST} mkdir-sandbox HDAA_RFC2069"

	cd src
	for p in ${PATCHLIST}; do
		epatch "${FILESDIR}/${PN}-1.7.6.0-${p}.patch"
	done
